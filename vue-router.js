   ...
   */
    ...
    var HTML5History = function () {
        ...
        HTML5History.prototype.go = function go(path, replace, append) {
            ...
            path = url    // strip hash so it doesn't mess up params
.replace(hashRE, '')    // remove root before matching
.replace(this.rootRE, '');
            this.onChange(path, null, hash);
        };
        HTML5History.prototype.formatPath = function formatPath(path, append) {
            return path.charAt(0) === '/'    // absolute path
 ? this.root ? this.root + '/' + path.replace(/^\//, '') : path : resolvePath(this.base || location.pathname, path, append);
   ...
   */
    function activate(view, transition, depth, cb, reuse) {
        ...
        if (reuse) {
            ...
            component = view.build({ _meta: { $loadingRouteData: loading } });
            // handle keep-alive.
   ...
   */
    ...
    function View(Vue) {
        var _ = Vue.util;
        var componentDef = // 0.12
        Vue.directive('_component') || // 1.0
    ...
    function Link(Vue) {
        ...
        Vue.directive('link', {
            ...
            updateClasses: function updateClasses(path) {
                ...
                // add new class
                if (this.exact) {
                    if (dest === path || // also allow additional trailing slash
        ...
        Router.prototype._addRoute = function _addRoute(path, handler, segments) {
            ...
            if (handler.subRoutes) {
                for (var subPath in handler.subRoutes) {
                    ...
                    this._addRoute(subPath, handler.subRoutes[subPath], // pass a copy in recursion to avoid mutating
                    // across branches
                    segments.slice());
                }
            }
